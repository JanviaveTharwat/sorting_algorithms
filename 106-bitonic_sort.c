 ...
 * Return :void
 */
void bitonic_merg(int *array, size_t size, size_t start, size_t seq,
		char flow)
{
	size_t i;
	size_t jump = seq / 2;
	...
	if (seq > 1)
	{
		...
		bitonic_merg(array, size, start, jump, flow);
		bitonic_merg(array, size, start + jump, jump, flow);
	}
}
...
void bitonic_seq(int *array, size_t size, size_t start, size_t seq, char flow)
{
	...
	if (seq > 1)
	{
		...
		bitonic_merg(array, size, start, seq, flow);
